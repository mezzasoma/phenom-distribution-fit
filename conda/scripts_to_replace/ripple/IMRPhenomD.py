###############################################
### Source repository: https://github.com/tedwards2412/ripple
### Original file path: ripple/src/ripple/waveforms/IMRPhenomD.py
###############################################
import jax
...
def get_inspiral_phase(fM_s: Array, theta: Array, coeffs: Array, lambda_sigma: Array) -> Array:
    ...
    # Default sigma_2 coefficients were: 
    ...
    # Default sigma_3 coefficients were:
    ...
    # Default sigma_4 coefficients were:
    ...
    * (
    ...
    # Activated all coeffs of sigma_4
    sigma_4 = (
    lambda_sigma[22]
    + lambda_sigma[23] * eta
    + (chiPN - 1.0)
    * (
        lambda_sigma[24]
        + lambda_sigma[25] * eta
        + lambda_sigma[26] * (eta ** 2.0)
    )
    + (chiPN - 1.0) ** 2.0
    * (
        lambda_sigma[27]
        + lambda_sigma[28] * eta
        + lambda_sigma[29] * (eta ** 2.0)
    )
    + (chiPN - 1.0) ** 3.0
    * (
        lambda_sigma[30]
        + lambda_sigma[31] * eta
        + lambda_sigma[32] * (eta ** 2.0)
    )
    )
